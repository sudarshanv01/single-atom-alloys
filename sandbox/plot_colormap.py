...
if __name__ == "__main__":
    ...
    data_from_dos_calculation = json.load(
        open(f"outputs/intermetallics_pdos_moments.json")
    )
    ...
    for j, metal_row in enumerate([FIRST_ROW, SECOND_ROW, THIRD_ROW]):
        ...
        for indexw, width in enumerate(width_values):
            ...
            chemi_energy = fitting_function.fit_parameters(
                final_params, parameters_metal["eps_d"]
            )
        ...
        cax = axh[j].contourf(
            width_values, parameters_metal["eps_d"], energy_matrix_hyb, levels=100
        )
        cbar = figh.colorbar(cax, ax=axh[j])
        axh[j].contour(
            width_values,
            parameters_metal["eps_d"],
            energy_matrix_hyb,
            levels=5,
            colors="tab:gray",
            linewidths=0.5,
        ...
        cax = axo[j].contourf(
            width_values, parameters_metal["eps_d"], energy_matrix_ortho, levels=100
        )
        ...
        cax = axc[j].contourf(
            width_values, parameters_metal["eps_d"], energy_matrix_tot, levels=100
        )
